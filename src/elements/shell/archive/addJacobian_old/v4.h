...
void TACSShellElement<quadrature, basis, director, model>::addJacobian(
  ...
  for (int quad_index = 0; quad_index < nquad; quad_index++) {
    ...
    A2D::Mat<TacsScalar, 3 * num_nodes, 3 * num_nodes> d2d, d2u;
    A2D::Mat<TacsScalar, 6, 6> d2d0xi;

    // TODO : could make new hextract_multi(...,) with VarTuple/vector for second arg so one call for repeat pvalue inputs
    // may be extra expensive here to get Hessians of size 12x12 iteratively.. (prob very slow with reverse mode AD)
    strain_energy_stack.hextract(d.pvalue(), d.hvalue(), d2d);
    strain_energy_stack.hextract(d.pvalue(), u.hvalue(), d2u);
    ...
    strain_energy_stack.hextract(gty.pvalue(), d.hvalue(), d2gtyd);
    strain_energy_stack.hextract(gty.pvalue(), u.hvalue(), d2gtyu);

    basis::addInterpTyingStrainHessian(pt, d2gty.get_data(), d2ety);    

    // replace TacsShellAddTypingDispCoupling
    // make a routine for this part..
